...
public class LunarCalendar {
    ...
    public static Map<String, LunarCalendar> cache = new HashMap<String, LunarCalendar>();
    ...
    static String[] monthChinese = new String[]{"正月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "十一月", "腊月"};
    ...
    public static Element getCalendarDetail(Date date) throws ParseException {
        ...
        LunarCalendar lunarCalendarUtil = null;
        if (false) {
            lunarCalendarUtil = cache.get(cacheKey);
        } else {
            lunarCalendarUtil = new LunarCalendar();
    ...
    public void calendar(int y, int m) throws ParseException {
        ...
        //清明颜色
        if (m == 3) elements.get(tmp1).color = "red";
